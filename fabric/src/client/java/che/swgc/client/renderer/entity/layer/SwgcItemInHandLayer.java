...
public class SwgcItemInHandLayer<M extends EntityModel<SwgcMobRenderState> & ModelWithArms> extends RenderLayer<SwgcMobRenderState, M> {
   public SwgcItemInHandLayer(RenderLayerParent<SwgcMobRenderState, M> renderer) {
      super(renderer);
   }
   ...
   protected void submitArmWithItem(
      ...
      if (!item.isEmpty()) {
         poseStack.pushPose();
         this.getParentModel().setArmAngle(arm, poseStack);
   ...
   private boolean useBabyOffset(SwgcMobRenderState state) {
